...
| `Components/ArtistTile.swift` | `designsystem/component/ArtistTile.kt` | obsolete | Landed in M2, **no call site since DS**: Discover, Search and Artist list all draw `Tile.kt`. Carries raw 192/252dp and `Color.White`; deletion tracked in #181 |
| `Components/MediaContainer.swift` | — | missing | **The row was wrong: `MediaContainer.kt` has never existed on any branch.** Media slots are drawn by `Tile.kt`'s `MediaSlot` and by the private cover composables in `feature/discover`, `feature/search` and `feature/profile` — which is itself a finding (#177, three copies of one cover) |
...
| `Components/ScoreRing.swift` | `designsystem/component/ScoreRing.kt` | obsolete | Ported with New-tier nil handling, but it has had **no call site since the Sep-2026 redesign** — `feature/score` draws its own `ScoreDonut`. Dead code, and the only raw literals in that section (64/6dp, 1.2sp); deletion tracked in #181 |
| `Components/Sparkline` | `designsystem/component/Sparkline.kt` | obsolete | **No call site.** The studio's trend is a hand-rolled 4.5dp meter, not this. Dead code (raw 48/3dp); deletion tracked in #181 |
| `Components/HeaderBar.swift` | `designsystem/component/Headers.kt` (`ScreenHeader` / `BackHeader`) + `feature/booking/BookingChrome.kt` (`FunnelBar`) | partial | Tab roots use `ScreenHeader` (8 sites), pushed screens `BackHeader` (27), the funnel `FunnelBar`/`FunnelStepBar` (4). Deliberately not a Material `TopAppBar`. **`ScreenTitleBar.kt` and `FunnelHeader` are dead** — the row named both until Sep 2026. `partial` because the three headers ship three bar heights and two subtitle steps, and 35 call sites still pass a 40dp back circle where §2 says 42 (#166) |
