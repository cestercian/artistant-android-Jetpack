...
**Android current (Sep 2026):** M0–M7 on `main`, then the **"Artistant iOS
Light" redesign** — P1 foundation (#141) and ten of the eleven P2 sections on
`main`: DS (#142), AP (#143), GS (#144, #149), MS (#145), BC (#146), BN (#147),
WZ (#148), AS (#150), PK (#151), SH (#152). **AC (Account & settings) is still in
flight**, so the Profile / Settings rows below are the ones to distrust.
`docs/REDESIGN_2026-09.md` is the plan and the token sheet. Every row that has
been through the redesign carries its section tag and its design numbers — a row
with neither has not, and saying which is what this file is for.
...
|---|---|---|---|
| `Screens/DiscoverView.swift` | `feature/discover/DiscoverScreen.kt` | **redesigned (DS)** | Sep-2026 light design, screens 02 + 59. Header (city + today) · `SearchBarButton` · category chips · one 262dp `HeroCard` · titled two-up `Tile` rails · `SkeletonPage`. Hero carousel and full-bleed insets retired; "See all" seeds Search via `SearchSeed`. The header circle is the design's **bell → `activity`**, its dot driven by `ActivityLog`'s unread count; Saved lost this borrowed slot and keeps its Profile row (26) |
...
| `Screens/GigRequestDetailView.swift` (counter sheet) | `feature/booking/CounterOfferScreen.kt` | **redesigned (BC)** | Screen 61, route `counter_offer/{requestId}` on the ARTIST graph — `gig_requests_update_artist` (mig 0002) is the only UPDATE policy, so a client-side counter is blocked on the backend. Since AS landed it is the ONLY way to counter: the gig detail's inline dialog is gone |
...
## Screens — Artist studio (section AS, design 09 / 85 / 86 / 133 / 36 / 35 / 107 / 108 / 109 / 22 / 105 / 106)

| iOS path | Android target | Status | Notes |
|---|---|---|---|
| `Screens/ArtistHomeView.swift` | `feature/artisthome/ArtistHomeScreen.kt` + `ArtistStudioLogic.kt` | **redesigned (AS)** | Screens 09 / 85 / 86. Money first, score second, requests third — the accent card leads with what the month has PLAYED plus what is agreed and still ahead; "awaiting settlement" is not said, because nothing is in custody. `dashboardMode` picks between three screens: a failed FIRST bookings read is **Unavailable** (em-dashes, inert strip, an explicit refusal), never Cold — an unshaded 14-day strip reads as fourteen open nights. Nothing on the page mutates: the design's inline Accept would commit the irreversible answer with the clash warning three taps away |
| — (new, design 133) | `feature/artisthome/EarningsScreen.kt` | **new (AS)** | "Earnings without custody" — twelve trailing months of AGREED fees, route `earnings`. Row states are Agreed / **Played**, not the design's Settled: settlement is not observable here. Buckets by gig date, excludes `pending_confirm` so declining a request cannot make earnings fall. Says outright it is not a payout statement |
| `Screens/ArtistGigsView.swift` | `feature/gigs/ArtistGigsScreen.kt` | **redesigned (AS)** | Screen 36, "calendar, then the day". The rewrite is a deletion — the month-grouped list below the grid is gone, so a booking no longer appears twice; the grid is the index and the list is one day. The selection is never null (`initialDayFor`). Day rows draw the clock stacked through the shared `ClockColumn` |
| `Screens/GigRequestDetailView.swift` | `feature/gigs/GigRequestDetailScreen.kt` + `GigRequestLogic.kt` | **redesigned (AS)** | Screens 35 / 107 / 108 / 109 behind one route. Clash warning sits ABOVE the dock — only one of the three answers is irreversible and there is no un-accept. Counter leaves for `counter_offer/{id}` (screen 61, BC). **Withdraw** (107) has no write path and **Message** (107/108) has no thread — one is created on ACCEPT (mig 0047) — so neither is drawn |
| — (design 21 Open gigs) | — | **blocked** | `gig_requests_select_participants` (mig 0002, narrowed by 0009) is `auth.uid() = artist_id or auth.uid() = client_id`, so an artist cannot read a request addressed to somebody else. A feed of other hosts' requests has no data source, and the match percentage has no column |
...
| `Components/ScoreRing.swift` | `designsystem/component/ScoreRing.kt` | obsolete | Ported with New-tier nil handling, but it has had **no call site since the Sep-2026 redesign** — `feature/score` draws its own `ScoreDonut`. Dead code, and the only raw literals in that section (64/6dp, 1.2sp); deletion tracked in #181 |
| `Components/Sparkline` | `designsystem/component/Sparkline.kt` | obsolete | **No call site.** The studio's trend is a hand-rolled 4.5dp meter, not this. Dead code (raw 48/3dp); deletion tracked in #181 |
...
| iOS path | Android target | Status | Notes |
|---|---|---|---|
| `Screens/ProfileView.swift` | `feature/profile/ProfileScreen.kt` | done | **Redesigned Sep 2026 (AC, design 26).** Client tab root only: identity, "Switch to artist mode" (one `users.role` write, then the root gate re-routes), the three-up band, and rows. Settings moved to `AccountScreen`. "Payment and billing" / "Invoices and GST" are not drawn — v1 takes no money and neither row has anything to open. The old Help/Feedback sheet is gone: section SH replaced it with real `help_centre` / `feedback` screens, and the account list's "Help centre" row pushes SH's — `FeedbackScreen` files into the same `app_feedback` behind an outbox the sheet had no notion of. "Saved artists" is now the ONLY route to `artist_list/saved`, Discover's header having taken the bell the design draws |
| `Screens/ProfileView.swift` (settings list) | `feature/profile/AccountScreen.kt` | done | **New Sep 2026 (AC, design 47/69).** ONE list, two hosts — the ARTIST group is injected by role, never forked. Stat band swaps columns per role. Route `account` on both graphs; the artist's old `profile` route is retired. Carries the entry point for every System screen the design gives none: **Activity** (123, above Notifications — the log, then the setting), and a support tail of Help centre, **Send feedback** (64), **What's new** (137, a request to the root's `WhatsNewViewModel`) and **Rate Artistant** (138, `AppStore.openListing`) |
| `Screens/Settings/DataExportView.swift` | `feature/profile/DataExportScreen.kt` | done | **Redesigned Sep 2026 (AC, design 49/81/82/113).** A sealed `ExportState` makes "failed with a file" unrepresentable; the expiry is the server's, never invented; a failure ships nothing |
| — (iOS delete is a dialog) | `feature/profile/DeleteAccountScreen.kt` | done | **New Sep 2026 (AC, design 48/115/116).** Reason → consequences → receipt. Stage 2 names every loss and only counts what it read; stage 3 itemises the 30-day backup window and leaves it unticked |
| — | `feature/profile/NotificationSettingsScreen.kt` | done | **New Sep 2026 (AC, design 124).** Eight DataStore toggles, marketing off by default, quiet hours named. `POST_NOTIFICATIONS` is a real read with a route to the system screen. No server-side preference exists — the screen says so |
| — | `feature/profile/AccessibilityScreen.kt` | done | **New Sep 2026 (AC, design 129).** Two real app-side controls (tab-bar labels, autoplay); text size / reduce motion / bold / contrast are system-owned on Android and are stated as facts with links, not faked as switches |
| — | `feature/profile/LanguageScreen.kt` | partial | **New Sep 2026 (AC, design 130).** Per-app locale via the platform `LocaleManager` (API 33+; no appcompat). Only English has resources, so the other five render with the reason and are not selectable. Region is three facts — India/INR only in v1 |
| — | `feature/profile/DevicesScreen.kt` | partial | **New Sep 2026 (AC, design 128).** This device from `Build`, plus "Sign out everywhere else" (`SignOutScope.OTHERS`). Supabase exposes no session list and there is no activity table, so neither is drawn — blocked on backend |
| — (no iOS screen) | Tax details (design 134) | blocked | No `legal_name`/`trading_as`/`pan`/`gstin`/billing-address columns on `users` or `artists`. `kyc_records.pan` is payout KYC (v1 has no payouts) and its siblings are `not null` |
| — (no iOS screen) | Invite a friend (design 125) | blocked | No referral/invite ledger anywhere in the 105 canonical migrations. Sent/joined/earned cannot be read, so the screen is not shipped |
| `Screens/Settings/ManageAvailabilityView.swift` | `feature/availability/ManageAvailabilityScreen.kt` | **redesigned (AS)** | Screens 22 / 105 / 106, "the calendar is inventory". The month leads, drawn with the SHARED `MonthDayGrid` (it gained one optional `unavailableDays` parameter rather than a second grid that disagreed about a booked day) and no selectable day. The preview shows the MISSING badge on an empty week (105). Two refusals: a failed bookings read marks nothing as taken and says so, and a failed seed disables Save (106) so an empty working copy cannot wipe real data |
| `Screens/Settings/ScoreHistorySheet.swift` | `feature/score/ScoreHistoryScreen.kt` | done | **Redesigned Sep 2026 (design 51)** — a pushed screen, not a sheet. Per-*recomputation* deltas: `score_history` stores no reason column, so no per-event cause is invented. Route `score_history` |
| `Screens/PaywallView.swift` | `feature/paywall/PaywallScreen.kt` | done | **Redesigned Sep 2026 (AC, design 25/91/92/93).** Four states derived by `proStateFor` from `EntitlementStore` + the store query, so an outage can never read as a lost plan. One plan card (Play carries one product); the active screen drops the design's fee arithmetic, which describes a product that moves money |
...
| 137 What's new | `feature/system/WhatsNewSheet.kt` + `ReleaseNotes.kt` + `WhatsNewViewModel.kt`, presented from the root | done | Once per `BuildConfig.VERSION_NAME` (DataStore) from a compiled notes table — there is no releases table, and notes describe the binary so they ship with it. A null seen-version records silently: greeting a fresh install with "what's new" is the worse error. Over the tab shell only — `WhatsNewHost()` in `ArtistantNavHost`, composed whenever the gate is `Tabs` and nothing blocks. The account list's row calls `showOnDemand()` on that same root-scoped VM (a scaffold sits above any NavHost, so `hiltViewModel()` resolves the activity's), ignoring the seen-version record and falling back to the most recent authored note |
| 138 Rate Artistant | `feature/system/RatePromptSheet.kt` + `RatePrompt.kt`, presented from `ClientTabsScaffold` | done | Armed ONLY by a completed booking review, asked once, never on launch. The trigger is resolved in the scaffold by watching the `ReviewSheetViewModel` the booking screen already takes as a parameter, so `feature/booking` is untouched. No Play in-app review library (no new Gradle deps): the button opens the store listing and the copy says so. The account list's "Rate Artistant" row opens the same listing directly — a way in for somebody who did not wait to be asked, without re-arming a prompt whose point is when it appears |
...
| `State/RequestStore.swift` | `ArtistHomeViewModel` + `GigRequestDetailViewModel` (+ RequestsRepository) | done | Requests list on the studio dashboard and the four-state request screen; the derivations are pure in `GigRequestLogic.kt` (AS) |
...
| `Components/CardView.swift` | `designsystem/component/CardView.kt` | obsolete | **No product call site** — reached only from `ComponentGallery`, which is itself unreachable. Dead code; deletion tracked in #181 |
| `Components/HRule` / Section | `HRule.kt` | partial | |
| `Components/ArtistTile.swift` | `designsystem/component/ArtistTile.kt` | obsolete | Landed in M2, trimmed in `3469d09`, and **no call site since DS**: Discover draws `Tile.kt`, Search its own `OccasionCard`/`ResultCard`/`ArtistThumb`, Artist list its own `ArtistThumb`. Carries raw 192/252dp and `Color.White`; deletion tracked in #181 |
| `Components/MediaContainer.swift` | ~~`designsystem/component/MediaContainer.kt`~~ | obsolete | Ported in `6e12b3a` (M2b), softened in `1a49277`, then **deleted in `3469d09`** ("resolve the three components nothing called"). Media slots are drawn by `Tile.kt`'s `MediaSlot` and by the private `ArtistThumb`/cover composables in `feature/search` and `feature/profile` — three copies of one cover, which is #177 |
...
| `Components/ScoreRing.swift` | `designsystem/component/ScoreRing.kt` | obsolete | Ported with New-tier nil handling, but it has had **no call site since the Sep-2026 redesign** — `feature/score` draws its own `ScoreDonut`. Dead code, and the only raw literals in that section (64/6dp, 1.2sp); deletion tracked in #181 |
| `Components/Sparkline` | `designsystem/component/Sparkline.kt` | obsolete | **No call site.** The studio's trend is a hand-rolled 4.5dp meter, not this. Dead code (raw 48/3dp); deletion tracked in #181 |
| `Components/HeaderBar.swift` | `designsystem/component/Headers.kt` (`ScreenHeader` / `BackHeader`) + `feature/booking/BookingChrome.kt` (`FunnelBar`) | partial | Tab roots use `ScreenHeader` (6 production call sites), pushed screens `BackHeader` (25), the funnel `FunnelBar` + `FunnelStepBar` (4) — counts exclude previews and the unreachable gallery. Deliberately not a Material `TopAppBar`. **`ScreenTitleBar.kt` and `FunnelHeader` are dead**; the row named both as the live targets until Sep 2026. `partial` because the three headers ship three bar heights and two subtitle steps, and the 40dp `iconCircleSm` backs roughly 13 of its 36 uses as a header back control where §2 says 42 (#166) |
...
| Component library v2 | `designsystem/component/*` | done | `PrimaryButton`, `SecondaryButton`, `IconCircle`, `SearchBar`/`SearchBarButton`, `Chip`/`ChipRail`, `SectionHeader`/`EyebrowLabel`, `Tile`/`MediaSlot`, `HeroCard`, `ListRow`, `Banner` (absorbs `InlineBanner`), `StatusPill`, `EmptyState`, `Skeleton*`, `Toast`/`ToastHost`, `LightTabBar`, `ScreenHeader`, `BackHeader`, `SheetScaffold`, `AppTextField`, `OtpField`. Landed in P1 with previews and no call sites; the P2 sections wired them and added their own (`DetailHeader`, `EventTimeline`, `BottomActionBar`, `AccentNote`, `ClockColumn`, `GroupLabel`, `SendingNarration`, `DashedSlot`, `TrustedTick`, `Meter`) |
| Month calendar | `designsystem/component/MonthCalendar.kt` | **redesigned (BN)** | Design 78 documents all five day states and this implements them: `MonthDayFill` (Booked · Unavailable · Open · Selected) plus today's ring, with `monthDayFill` as the pure precedence rule. Adds `MonthCalendarCard`, `MonthCalendarLegend`, `DayEventRow` and `ClockColumn` — the stacked "8:00" over "pm" time column designs 36 and 78 draw identically, shared with the Gigs day list. `MonthDayGrid` keeps its signature for Gigs and gains one optional `unavailableDays` |
| Section SH primitives | `designsystem/component/{Toast,SectionHeader,LightTabBar}.kt` | **extended (SH)** | `ToastHost` takes a `key` (identity of the current message, so a repeated string restarts the timer); `GroupLabel` joins `EyebrowLabel` — the design sets an eyebrow in JetBrains Mono and a list's group divider ("TODAY", "ALSO FIXED") in the sans at 12.5/700, and a run of mono dividers makes a scrolling list read as a log file; `lightTabBarHeight()` exposes the bar's height from its own tokens for the one caller that must clear it without being laid out against it. The host above the NavHost is the ONLY one — a feature raises a toast by injecting the `@Singleton ToastController` into its ViewModel (`ArtistProfileViewModel` does), never by mounting a second host. `feature/epk` still has one, and is the outstanding migration |
